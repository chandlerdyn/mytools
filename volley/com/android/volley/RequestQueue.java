...
public class RequestQueue {
    ...
    /** The cache triage queue.<p>缓存队列 */
    private final PriorityBlockingQueue<Request<?>> mCacheQueue =
        new PriorityBlockingQueue<Request<?>>();

    /** The queue of requests that are actually going out to the network. <p>网络请求队列*/
    private final PriorityBlockingQueue<Request<?>> mNetworkQueue =
        new PriorityBlockingQueue<Request<?>>();

    /** Number of network request dispatcher threads to start.<p>默认的dispatcher线程数量 */
     ...
     * <p>start 方法中，开启一个缓存调度线程CacheDispatcher和 n 个网络调度线程NetworkDispatcher，这里 n 默认为4，
     ...
     *  <p>(1). 首先从正在进行中请求集合mCurrentRequests中移除该请求。
